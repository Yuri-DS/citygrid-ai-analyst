...
from agent.torch_patch import safe_torch_classes_patch
...
class CityGridAgent:
    ...
    # Tools that consume SQL data (visualizations + analytics)
    DATA_CONSUMING_TOOLS = {"create_chart", "create_map", "detect_anomalies"}
    ...
    # Checklist uses only workflow tools; exclude helper (get_table_sample)
    # so the planner never suggests "sample 5 rows" for map/chart requests.
    CHECKLIST_TOOLS = {"search_documentation", "execute_sql", "create_chart", "create_map", "detect_anomalies"}
    ...
    def _generate_checklist(self, question: str) -> list[str]:
        ...
        try:
            ...
            # Ensure execute_sql before first data-consuming tool (viz or anomaly)
            data_tools = self.DATA_CONSUMING_TOOLS
    ...
    def _agent_node(self, state: AgentState) -> dict:
        ...
        # (e.g. during message handling). Patch is isolated in agent.torch_patch.
        with safe_torch_classes_patch():
    ...
    def _get_skipped_steps(
        self,
        checklist: list[str],
        checklist_done: dict[str, bool],
        tool_name: str,
    ) -> list[str]:
        """Return checklist steps skipped before the current tool."""
        if not checklist or tool_name not in checklist:
            return []
    ...
    def _find_all_blocked_redirect(
        self,
        tool_calls: list[dict],
        checklist: list[str],
        checklist_done: dict[str, bool],
    ) -> str | None:
        """Return first required checklist step when all tool calls are blocked."""
        checklist_redirect_to = None
        all_blocked = True

        for tc in tool_calls:
            tc_name = tc["name"]
            skipped_pre = self._get_skipped_steps(checklist, checklist_done, tc_name)
            if skipped_pre:
                if checklist_redirect_to is None:
                    checklist_redirect_to = skipped_pre[0]
                continue
            all_blocked = False
            break

        if all_blocked:
            return checklist_redirect_to
        return None

    def _build_all_blocked_response(
        self,
        last_message: AIMessage,
        checklist: list[str],
        checklist_redirect_to: str,
        checklist_done: dict[str, bool],
    ) -> dict:
        """Build response when all tool calls violate checklist order."""
        self.logger.log("checklist_order_violation", {
            "called": [tc["name"] for tc in last_message.tool_calls],
            "redirected_to": checklist_redirect_to,
            "action": "removed_from_context",
        })
        checklist_done_out = dict(checklist_done or {})
        self.logger.log("checklist_done_updated", {
            "checklist_done": checklist_done_out,
            "checklist": checklist,
        })
        return {
            "messages": [
                ...
                HumanMessage(content=(
                    f"Call {checklist_redirect_to} now. "
                    f"Do NOT guess table or column names — read the documentation first, "
                    f"then write your query from scratch based on what you find."
                )),
            ],
            "checklist_done": checklist_done_out,
        }

    def _build_skip_warning_message(
        self,
        tool_name: str,
        skipped: list[str],
        tool_call_id: str,
    ) -> ToolMessage:
        """Build warning ToolMessage when tool call skips required checklist step."""
    ...
    def _inject_viz_data_if_missing(
        self,
        tool_name: str,
        tool_args: dict,
        ...
        """Fill data from previous SQL result for data-consuming tools, or return an error ToolMessage."""
        if tool_name not in self.DATA_CONSUMING_TOOLS:
            return None
    ...
    def _format_tool_result_for_llm(self, tool_name: str, result: Any) -> str:
        """Format tool result for LLM, preserving existing behavior."""
        ...
        if tool_name == "execute_sql" and isinstance(result, dict) and result.get("success") and result.get("data"):
            ...
            truncated_result = {
                ...
                "message": f"Result has {result['row_count']} rows; first {len(trunc)} shown. Full data available for create_map/create_chart/detect_anomalies." if len(result["data"]) > max_rows_for_llm else None,
            }
            truncated_result = {k: v for k, v in truncated_result.items() if v is not None}
            return json.dumps(truncated_result, ensure_ascii=False)
    ...
    def _tools_node(self, state: AgentState) -> dict:
        ...
        if hasattr(last_message, "tool_calls") and last_message.tool_calls:
            ...
            # When ALL calls are blocked, we remove the AIMessage entirely from
            # context to prevent anchoring bias (the LLM tends to reuse its own
            # wrong SQL from the blocked attempt instead of writing from scratch).
            checklist = state.get("checklist") or []
            checklist_done = state.get("checklist_done") or {}

            checklist_redirect_to = self._find_all_blocked_redirect(
                last_message.tool_calls,
                checklist,
                checklist_done,
            )
            if checklist_redirect_to:
                return self._build_all_blocked_response(
                    last_message,
                    checklist,
                    checklist_redirect_to,
                    checklist_done,
                )
            ...
            for tool_call in last_message.tool_calls:
                ...
                skipped = self._get_skipped_steps(checklist, checklist_done, tool_name)
                if skipped:
                    results.append(self._build_skip_warning_message(tool_name, skipped, tool_call_id))
                    continue

                viz_error = self._inject_viz_data_if_missing(tool_name, tool_args, messages, tool_call_id)
                if viz_error is not None:
                    results.append(viz_error)
                    continue
                ...
                if tool_fn:
                    try:
                        ...
                        llm_result_str = self._format_tool_result_for_llm(tool_name, result)
