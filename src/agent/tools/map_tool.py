...
def create_map(
    ...
    Args:
        ...
        map_type: "points" for markers, "lines" for connected lines.
                 For lines, data must have start_lat, start_lon, end_lat, end_lon.
        title: Map title.
    ...
    Examples:
        ...
        # Road network (lines)
        create_map(data, lat_column="start_lat", lon_column="start_lon",
    ...
    try:
        ...
        if map_type == "lines":
            center_lat = df[["start_lat", "end_lat"]].mean().mean()
            center_lon = df[["start_lon", "end_lon"]].mean().mean()
        else:
