...
|-------|-------------|-------|
| districts | Administrative areas | Small table |
| city_objects | Physical infrastructure | Filter by object_type |
| road_network_nodes | Road graph nodes | Small table |
| sensors | IoT sensors | Join with city_objects for location |
| smart_meters | Utility meters | Small table |
| sensor_readings | Sensor time series | LARGE — always filter by time! |
| meter_readings | Meter time series | LARGE — always filter by time! |
| citizen_requests | Complaints/311 | Medium table |
| municipal_events | City events | Small table |
| public_transport_trips | Transport data | Medium-large table |
